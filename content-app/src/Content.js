...
export default function Header(props) {
  ...
  useEffect(() => {
    console.log("Content: incoming props :>> ", props);
    listenMessage((e) => {
      const { to, message } = e.detail;
      if (to === "content") setRootMessage(message);
    });
  }, [props]);

  const sendMessageToChilds = () => {
    sendMessage("header", childMessage);
  };

  return (
    <section className="container mx-auto mt-10 flex flex-col space-y-5 p-20 pt-10 pb-12 bg-orange-200 text-gray-500 border-2 border-dashed border-gray-700">
